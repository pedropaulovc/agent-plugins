...
A single script (`watch-pr.py`) runs through Claude Code's **Monitor** tool, OpenCode's event-driven plugin bridge, or a background terminal fallback. It diffs PR state every 30s, emitting actionable lifecycle lines — aggregated CI check waves, `rebase` state (BEHIND/DIRTY), `review`, `comments`, `reaction` (Codex 👀→👍), unresolved-comment deltas, and terminal `finished`. The `SKILL.md` maps each event to an action: investigate named CI failures, `git pull --rebase` when behind, and — when feedback changes — open the generated file for reply drafting.

CI pending/pass/skipping/cancel churn is coalesced into one rerun-start event and one all-terminal summary; newly failing checks remain immediate and named. On new or reopened inline threads the script emits **one compact `feedback …` line per thread** plus a pointer to the full markdown file. Top-level comment and body-review summaries are deliberately not emitted inline. The pointer and thread lines are deduplicated against unchanged formatter output, so a later feedback trigger does not repeat old content. It fetches once on startup and re-fetches when review/comment state changes or unresolved threads are added. Self-terminates on MERGED/CLOSED.
