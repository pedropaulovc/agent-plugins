#!/usr/bin/env python3
"""
Build script: cross-compiles the powershell-autofix Rust binary for
...
Prerequisites (same as the `personal` plugin's build-hooks.py):
    ...
    for crate_name in CRATES:
        ...
        for target in PLATFORM_TARGETS:
            try:
                ...
                print(
                    f"WARNING: failed to build {crate_name} for {target['triple']}, skipping",
                    file=sys.stderr,
                )
                continue
