...
const binary = path.join(pluginRoot, "hooks/bin/windows-bash-guard.exe");

export const WindowsBashGuardPlugin = async () => {
  const notices = new Map();
  return {
    "tool.execute.before": async (input, output) => {
      if (input.tool !== "bash" || process.platform !== "win32") return;
