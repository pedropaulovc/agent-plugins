...
For every `Read` tool call in those transcripts whose target resolved to a file under `memory/` (excluding the `MEMORY.md` index itself), it records one `{sessionId, memoryFileName}` pair, deduplicated per session, and (re)writes the full set to `./memory/usage.jsonl` — one JSON object per line. The file is fully regenerated on each run rather than appended to, so it self-heals if a memory file is renamed or removed.
