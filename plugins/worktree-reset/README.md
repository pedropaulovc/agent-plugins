...
- `--force` discards tracked, untracked, ignored, and repository-wide stashed changes
  ...
  worktree. It synchronizes submodules recursively and reports failure instead of success
  when the root worktree or any submodule remains dirty or out of sync.
...
branches, branch resets, recursive submodule synchronization, final cleanliness verification,
and `npm install`, `go mod download`, and `uv sync --locked` when the corresponding files exist.
