...
# Escape hatch: the agent put [force-fetch] anywhere in the tool call (the url is
# the documented spot). Honor it, but STRIP the marker from the request first so
# it never reaches the network as part of the URL. Reserved for the worst cases
# (e.g. the agent is restricted to the fetch tool only and has no MCP
# alternative); not a routine bypass.
if echo "$input" | grep -q '\[force-fetch\]'; then
  if command -v jq >/dev/null 2>&1; then
    printf '%s' "$input" | jq -c '{
      hookSpecificOutput: {
        hookEventName: "PreToolUse",
        updatedInput: (.tool_input | walk(if type == "string" then gsub(" ?\\[force-fetch\\] ?"; "") else . end)),
        additionalContext: "no-fetch: [force-fetch] escape hatch honored; the marker was stripped from the request before fetching."
      }
    }'
  fi
  exit 0
